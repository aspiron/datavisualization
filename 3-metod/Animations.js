...
function init()
{
	...
	startNode = createCircleNode(new Rect(22.5, 10, 35, 15), 'Bosing');
	...
	createRectangleNode(new Rect(85, 10, 60, 15), '1-guring');
	createRectangleNode(new Rect(85, 40, 60, 15), '2-guring');
	createRectangleNode(new Rect(10, 40, 60, 15), '3-guring');
	...
	ArrayList.forEach(diagram.nodes, function (node)
	{
		...
		nodeAnimations.push(new Animation(node,
		{
			...
			animationType: +$('#animationType')[0].value,
			easingType: +$('#easingType')[0].value
		}, onUpdateNode));
	});
}
...
function onAnimationTypeChanged()
{
	...
	ArrayList.forEach(nodeAnimations, function (a)
	{
		var aType = 1;
...
function onEasingTypeChanged()
{
	...
	{
		if (!a.isRunning())
			a.setEasingType(+$('#easingType')[0].value);
	});
}
...
{
	diagram.clearAll();
	ArrayList.forEach(linkAnimations, function (a)
	{
		if (a && a.isRunning())
			a.stop();
	});
