...
$(document).ready(function ()
{
	...
	var node = diagram.getFactory().createContainerNode(5, 10, 60, 30);
	node.setText("Bilaman");

	node = diagram.getFactory().createContainerNode(5, 50, 60, 30);
    node.setText("Bilishni xohlayman");
	
    node = diagram.getFactory().createContainerNode(5, 90, 60, 30);
	node.setText("Bilib bo`ldim");
});
...
function onDiagramDrop(e)
{
	...
	{
		var node = diagram.getFactory().createShapeNode(0, 0, 40, 10);
		node.setText(data);
		node.setLocked(true);
        
		container.add(node);
        
		container.arrange(new MindFusion.Graphs.LayeredLayout('TopToBottom'));
//        container.setDirection('TopToBottom')
	}
}
